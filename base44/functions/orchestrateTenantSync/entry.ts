...
// getLastWatermark / saveCheckpoint removed — all steps now use ensureSyncState / markSyncSuccess
...
async function syncLookup(sr, relayUrl, relaySecret, force, config) {
   const { step, entityName, relayEndpoint, pageSize, mapRow, dedup } = config;
   const syncKey = `global:${step}`;
   const syncState = await ensureSyncState(sr, { syncKey, tenantId: null, step, force });
   const isFullSync = syncState.requested_mode === 'full';
   const lastWatermark = isFullSync ? null : (syncState.watermark_timestamp ? new Date(syncState.watermark_timestamp) : null);
   let maxWatermark = lastWatermark;
   let offset = 0;
...
async function runAllLookups(sr, relayUrl, relaySecret, force, tenantId, syncRunId, isFullSync) {
  const lookups = [
    {
      step: 'lookupGhsHazardCodes', entityName: 'GHSHazardCode',
    ...
    {
      step: 'lookupGhsPictograms', entityName: 'GHSPictogram',
    ...
    {
      step: 'lookupGhsPrecautionary', entityName: 'GHSPrecautionaryStatement',
    ...
    {
      step: 'lookupPPE', entityName: 'PPEReference',
    ...
    {
      step: 'lookupNFPA', entityName: 'NFPAGuide',
    ...
    {
      step: 'lookupGlossary', entityName: 'GlossaryTerm',
