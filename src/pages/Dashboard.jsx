...
import { FlaskConical, AlertTriangle, Star, Building2, RefreshCw, ChevronRight, Loader2 } from 'lucide-react';
import { useTenant } from '@/lib/TenantContext';
import TenantGate from '@/components/shared/TenantGate';
import IncrementalSyncProgress from '@/components/shared/IncrementalSyncProgress';
...
function DashboardInner() {
  const { tenantId, syncStatus, syncMessage, isSyncLocked } = useTenant();
  const [registry, setRegistry] = useState([]);
  const [sites, setSites] = useState([]);
  const [loading, setLoading] = useState(true);
  const [syncing, setSyncing] = useState(false);
  const [syncMode, setSyncMode] = useState(null);
  const [registrySyncStatus, setRegistrySyncStatus] = useState(null);
  ...
  useEffect(() => {
    ...
    const fetchAll = async () => {
      try {
        ...
        // Check current sync mode and status
        const syncKey = `tenant:${tenantId}:registry`;
        try {
          const stateRaw = await base44.entities.SyncState.filter({ sync_key: syncKey });
          const stateArr = Array.isArray(stateRaw) ? stateRaw : (stateRaw?.data || []);
          setSyncMode(stateArr[0]?.requested_mode || null);
          setRegistrySyncStatus(stateArr[0]?.status || null);
        } catch (e) {
          console.error('Failed to fetch SyncState:', e);
        }
  ...
  return (
    <div className="p-6 space-y-6 max-w-7xl mx-auto">
         ...
         <div className="flex items-center gap-2">
           ...
           {syncMode === 'full' && (
             <Button
               ...
               size="sm"
               onClick={async () => {
                 if (!confirm('⚠️ This will perform a full resync. Continue?')) return;
                 setSyncing(true);
                 try {
                   await base44.functions.invoke('orchestrateTenantSync', { site_parent: tenantId, force: true });
                 } catch (e) {
             ...
             >
               {syncing || isSyncLocked ? (
                 <><Loader2 size={14} className="mr-1.5 animate-spin" />{syncMessage || 'Syncing…'}</>
               ) : (
                 <><RefreshCw size={14} className="mr-1.5" /> Force Resync All</>
               )}
             </Button>
           )}
      ...
      {/* Incremental Sync Progress Banner */}
